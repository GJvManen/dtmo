...
Current state after `RUN-20260806-011`:
...
- controlled pull-request execution probe: created on branch `pdca/run-20260806-011-ci-trigger`;
- pull-request-triggered quality-gate execution: `PENDING` until independently visible;
...
- external control if the probe produces no run: repository Actions permissions and workflow enablement;
