...
Repository CI proves the canonical wiring and preservation of the server-side readiness boundary. Live upstream retrieval is intentionally not fabricated or route-mocked in repository browser acceptance. The next external owner functional retest must prove the real clean-install operator sequence against the supported runtime: open `Sources & Collection`, select CISA KEV, observe manual-load readiness, run the feed, then verify resulting intelligence through the canonical downstream workspaces.
