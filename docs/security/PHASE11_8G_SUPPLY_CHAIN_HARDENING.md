...
flowchart LR
    ...
    IMAGE --> ISBOM[Trivy-generated CycloneDX container SBOM]
    WHEEL --> V1[Vulnerability gate]
    ISBOM --> V2[HIGH/CRITICAL vulnerability gate]
...
- The candidate container's runtime package identity is verified before scanning.
- Trivy generates the candidate container CycloneDX SBOM from the final built image; that Trivy-owned SBOM is then the provenance-bound input to the known-vulnerability gate so stale third-party SBOM metadata cannot silently override the final image package inventory.
- The Trivy-owned container SBOM is scanned for known `HIGH` and `CRITICAL` OS/library vulnerabilities; detected governed findings fail closed. No vulnerability waiver or ignore list is introduced by this flow.
...
1. **PR engineering evidence:** exact-head contract tests, Trivy-owned SBOM generation, vulnerability scan output and build hashes.
