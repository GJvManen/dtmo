...
- [RUN-20260809-136 — bounded search-health alerting](runs/RUN-20260809-136.md) — `CI_VALIDATION_PENDING`: PR #92 prerequisite accepted after 39/39 exact-head workflows and independent artifact inspection; RC10.6 implements bounded OpenSearch cluster-health-only alerting with 2-unhealthy raise / 2-healthy clear behavior, safe correlation, repeat-raise suppression, Prometheus rule/metrics and a dedicated retained-evidence workflow.
- [RUN-20260809-135 — bounded API-error alerting](runs/RUN-20260809-135.md) — `PASS`: PR #92 exact head `659fa022840e01ed6db4ebeb6a5e703f58a6d259` passed 39/39 workflows; artifact `9041987610` (`sha256:6a6f2aa5ea2b0b3fb081a0b376f8187a799af726ba950bcbf6fd8618c54e2eca`) independently showed JUnit 6/6 and exact-head machine-readable PASS evidence; merged as `8d6297e17c93150dacb39428ed3580e7c8cc1579`.
...
Phase 1–5 internal roadmap gates are `PASS`. Phase 6 remains `BLOCKED_EXTERNAL` only for genuine VoiceOver/NVDA behavior. Phase 7 is `IN PROGRESS`; RC10.1–RC10.5 and the bounded object-storage migration/reconciliation are accepted. RC10.6 search-health alerting is implemented but remains exact-head CI/artifact gated.

Fresh OpenSearch first-party review confirms 2.19 remains in maintenance and 2.19.6 is the current 2.x maintenance release with security updates. This does not close issue #1's separate production OpenSearch hardening/version gate.
...
Verify the complete exact-head workflow matrix and retained `search-health-alerting-evidence` artifact for RUN-136; merge only after every registered workflow succeeds and the artifact is exact-head bound and internally consistent.
