...
- [RUN-20260810-163 — RC6 frontend RC9 acceptance-contract regression remediation](runs/RUN-20260810-163.md) — `CI_VALIDATION_PENDING`: PR #112 exact head `0e6bc86b425b4e6511520bd6734f79baf7413d97` failed 11 RC9 workflow-level gates / 22 checks. Direct logs showed shared semantic-state, live-region, audit-row identity, visible-focus, responsive/reflow and contrast-measurement regressions. The remediation restores the historically accepted RC9 contracts without changing server-side RBAC, separation of duties, privacy, audit persistence or human share approval. A complete fresh exact-head matrix is required.
- [RUN-20260810-162 — 16.0.0rc6 professional frontend UX overhaul](runs/RUN-20260810-162.md) — `FAILED_CI`: the professional task-oriented UX was implemented, but first exact head `0e6bc86b425b4e6511520bd6734f79baf7413d97` is not accepted because 11 registered RC9 workflows failed. RUN-163 owns the bounded remediation; no PASS claim from RUN-162 is retained.
...
Phase 1–5 are `PASS`. Phase 6 remains externally blocked for genuine VoiceOver/NVDA execution; rc5 is the last accepted frontend baseline while rc6 PR #112 is under RUN-163 remediation and is not accepted. Phase 7 is `PASS`. Phase 8 remains `BLOCKED_EXTERNAL` for one approved real staging deployment and all ten deployment-parity evidence classes. Phase 9 remains `NOT COMPLETE`; its repository-controlled readiness/intake contract is accepted. Phase 10 remains `NOT STARTED`. DTMO is not production ready.
