...
- [RUN-20260806-043 — RC6.4 combined multi-store recovery acceptance](runs/RUN-20260806-043.md) — `CI_VALIDATION_PENDING`: one recovery-point binding, cross-store provenance controls, deterministic evidence binding and measured end-to-end RTO/RPO are committed
...
`RUN-20260806-043` is `CI_VALIDATION_PENDING`. The combined verifier and recovery workflow are committed, but no exact-head multi-store workflow or retained aggregate artifact has completed successfully.
