...
The active bounded programme step is **Phase 11.8c ingress/TLS and network segmentation**. Earlier Phase 8/9 evidence remains historical and candidate-bound. Fresh Phase 11.10 production-equivalent validation and Phase 11.11 independent external assurance remain required before Phase 12.
...
- [Phase 11.8c Ingress/TLS and Network Segmentation](architecture/PHASE11_8C_INGRESS_TLS_NETWORK_SEGMENTATION.md) defines the active north-south trust boundary.
- [Ingress/TLS and Network Segmentation Administration](administration/INGRESS_TLS_NETWORK_SEGMENTATION.md) defines TLS-only enablement and explicit controller selectors.
- [Phase 11.8c Ingress/TLS and Network Runbook](operations/PHASE11_8C_INGRESS_TLS_NETWORK_SEGMENTATION_RUNBOOK.md) defines live validation, failure handling and rollback.
- [Phase 11.8c Ingress/TLS and Network Segmentation Gate](qa/PHASE11_8C_INGRESS_TLS_NETWORK_SEGMENTATION_GATE.md) defines exact-head repository acceptance.
...
## Phase 11.8c trust boundary

```mermaid
flowchart LR
    C[External client] -->|TLS| IC[Approved ingress controller]
    IC -->|namespace + pod selectors| N[DTMO NetworkPolicy]
