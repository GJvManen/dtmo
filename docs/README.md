...
The active bounded programme step is **Phase 11.8d HA and disruption hardening**. Earlier Phase 8/9 evidence remains historical and candidate-bound. Fresh Phase 11.10 production-equivalent validation and Phase 11.11 independent external assurance remain required before Phase 12.
...
- [Phase 11.8b Workload Identity and External Secrets](architecture/PHASE11_8B_WORKLOAD_IDENTITY_SECRETS.md) records accepted 11.8b machine-identity and secret-delivery evidence.
...
- [Ingress/TLS and Network Segmentation Administration](administration/INGRESS_TLS_NETWORK_SEGMENTATION.md) defines the accepted TLS-only enablement and explicit controller selectors.
- [Phase 11.8c Ingress/TLS and Network Runbook](operations/PHASE11_8C_INGRESS_TLS_NETWORK_SEGMENTATION_RUNBOOK.md) defines live validation, failure handling and rollback for the accepted boundary.
- [Phase 11.8c Ingress/TLS and Network Segmentation Gate](qa/PHASE11_8C_INGRESS_TLS_NETWORK_SEGMENTATION_GATE.md) defines accepted repository evidence.
- [Phase 11.8d HA and Disruption Hardening](architecture/PHASE11_8D_HA_DISRUPTION.md) defines the active application scheduling/disruption and stateful-HA trust boundary.
- [Phase 11.8d HA and Disruption Runbook](operations/PHASE11_8D_HA_DISRUPTION_RUNBOOK.md) defines validation, failure handling and rollback without claiming live failover.
- [Phase 11.8d HA and Disruption Gate](qa/PHASE11_8D_HA_DISRUPTION_GATE.md) defines exact-head repository acceptance for this bounded slice.
...
## Phase 11.8d availability boundary
...
flowchart LR
    ...
    B --> S
    C --> S
    P[PodDisruptionBudget] -. constrains voluntary disruption .-> A
    P -. constrains voluntary disruption .-> B
    P -. constrains voluntary disruption .-> C
```

The model documents desired scheduling and disruption controls only. It does not prove live availability-zone placement, node or zone failure survival, stateful quorum/failover or provider durability.
...
- application replicas are spread across configured zone/host topology domains with fail-closed scheduling constraints;
- stateful HA is deployment-specific and is never inferred from stateless application scheduling;
