...
> **Active programme:** Phase 11 `IN PROGRESS / ACTIVE`  
...
Phase 11 evolves the platform by integrating mature open-source subsystems rather than expanding DTMO into a monolith. The fixed priority is Taranis AI → IntelOwl → OpenCTI → MISP consolidation → TheHive → Cortex only if justified → integrated runtime industrialisation.
...
See [System Architecture](docs/architecture/SYSTEM_ARCHITECTURE.md), [Taranis Platform Integration Assessment](docs/architecture/TARANIS_PLATFORM_INTEGRATION_ASSESSMENT.md) and [Security Overview](docs/security/SECURITY_OVERVIEW.md).
...
1. complete Phase 11.1 Taranis architecture/API/data-model/identity/licensing assessment;
2. implement the Taranis → DTMO canonical adapter;
3. integrate IntelOwl enrichment;
4. integrate OpenCTI STIX/graph capabilities;
5. consolidate MISP authority/synchronization;
6. add TheHive incident/case handoff;
7. add Cortex only when a validated IntelOwl gap requires it;
8. industrialise the composed runtime with Kubernetes/Helm/GitOps, HA, secrets, observability, recovery and supply-chain controls;
9. complete migration/compatibility;
10. execute new production-equivalent validation and independent external assurance;
11. conduct Phase 12 production GO/NO-GO.
...
Taranis AI is a separate EUPL-1.2 licensed project. Phase 11 uses a service-to-service integration boundary by default; Taranis source code must not be copied into DTMO before an explicit licensing review.
