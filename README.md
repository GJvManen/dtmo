...
- **Sources & Collection** — governed source registration, validation and collection control while keeping credentials server-side.
...
The repository contains container and Kubernetes/Helm deployment assets, GitOps-oriented runtime controls, workload identity/external-secret integration, ingress/TLS and network segmentation controls, availability/disruption policy, observability, backup/recovery, supply-chain controls, capacity planning and forward-first migration compatibility.
