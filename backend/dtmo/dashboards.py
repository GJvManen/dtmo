...
async def _seven_day_trend(session: AsyncSession) -> dict[str, int]:
    today = datetime.now(UTC).date()
    first_day = today - timedelta(days=6)
    counts = {(first_day + timedelta(days=offset)).isoformat(): 0 for offset in range(7)}
    discovered = (
        await session.scalars(
            select(IntelligenceItem.discovered_at).where(
                IntelligenceItem.discovered_at >= datetime.combine(first_day, datetime.min.time(), tzinfo=UTC)
            )
        )
    ).all()
...
async def dashboard_summary(
    ...
    trend = await _seven_day_trend(session)
    return {
...
<article class="surface chart"><h2>Intelligence trend — 7 dagen</h2><div id="trend-chart"></div><div id="trend-table"></div></article>
