...
def test_opencti_contract_records_upstream_api_data_and_license_boundary() -> None:
    ...
    required = (
        ...
        "STIX 2.1",
        "TAXII 2.1",
        "Access-controlled OpenCTI streams",
        "Apache License 2.0",
...
def test_opencti_contract_is_least_privilege_and_fail_closed() -> None:
    text = _read(CONTRACT)
    required = (
        "dedicated non-human OpenCTI identity",
        "no `Bypass all capabilities`",
        "runtime secrets",
        "`401` and `403` fail closed",
        "missing, malformed or unknown marking/TLP context fails closed",
