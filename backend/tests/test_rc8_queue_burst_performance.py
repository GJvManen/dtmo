...
def test_queue_burst_applies_backpressure_without_loss() -> None:
    budget = QueueBurstBudget(
        burst_records_per_second=250,
        connector_parallelism=4,
...
def test_queue_burst_fails_closed_when_recovery_budget_is_impossible() -> None:
    budget = QueueBurstBudget(
        burst_records_per_second=100,
        connector_parallelism=2,
        recovery_max_seconds=0.001,
        data_loss_max_records=0,
        duplicate_candidate_max_records=0,
    )
    result = asyncio.run(
        run_queue_burst_harness(
            ...
            consumer_records_per_second=10,
            queue_capacity=2,
        )
    )
