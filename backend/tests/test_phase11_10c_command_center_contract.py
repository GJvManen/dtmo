...
def test_integration_capabilities_reuse_governed_readiness_and_never_infer_runtime_health() -> None:
    settings = Settings(_env_file=None, feature_live_connectors=True, feature_taranis_connector=True, taranis_api_base="https://taranis.example.invalid/api", taranis_api_token="runtime-token", feature_intelowl_enrichment=True, intelowl_api_base="https://intelowl.example.invalid/api")
...
def test_command_center_maps_disabled_capabilities_to_actionable_administration_state() -> None:
    settings = Settings(_env_file=None, feature_opencti_read=False, opencti_api_base="https://opencti.example.invalid/graphql", opencti_api_token="runtime-token", opencti_allowed_entity_types="Indicator,Malware", opencti_checkpoint_path="state/opencti-checkpoint.json")
    opencti = next(item for item in build_integration_capabilities(settings) if item["id"] == "opencti")
...
def test_command_center_includes_ail_and_fails_closed_on_component_specific_readiness() -> None:
    settings = Settings(_env_file=None, feature_ail_connector=True, ail_api_base="https://ail.example.invalid/api", ail_api_key="runtime-secret")
...
def test_enabled_but_unconfigured_integration_fails_closed() -> None:
    ...
    cortex = next(item for item in build_integration_capabilities(settings) if item["id"] == "cortex")
...
async def test_canonical_store_failure_never_synthesizes_zero_metrics_or_trends() -> None:
    snapshot = await build_command_center_snapshot(cast(Any, _FailingSession()), Settings(_env_file=None))
    assert snapshot["data_state"] == "unavailable"
    assert snapshot["recent_intelligence"] == []
    assert snapshot["trends"] == {"intelligence_7d": [], "severity_distribution": [], "source_distribution": [], "type_distribution": [], "enrichment_status_distribution": [], "collection_volume_distribution": []}
...
def test_frontend_command_center_uses_governed_api_graphs_pivots_and_explicit_boundaries() -> None:
    ...
    for marker in ("/api/v1/command-center", "Command Center", "Canonical data unavailable", "Integration readiness", "Intelligence arrivals · 7 days", "Severity composition", "Open administration →", "Inspect collection", "Visibility ≠ authority", "Operational visibility without synthetic claims"):
...
def test_phase11_10c_documentation_and_gate_are_discoverable() -> None:
    expected = ("docs/architecture/PHASE11_10C_COMMAND_CENTER.md", "docs/qa/PHASE11_10C_COMMAND_CENTER_GATE.md", ".github/workflows/phase11-command-center.yml")
