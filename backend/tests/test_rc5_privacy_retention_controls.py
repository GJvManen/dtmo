...
def _event(*, occurred_at: datetime) -> object:
...
def test_minimized_projection_removes_direct_identity_and_token_identifiers() -> None:
    ...
    projection = minimize_audit_event(
    ...
    assert projection.event_hash == event.event_hash  # type: ignore[attr-defined]
...
def test_pseudonyms_are_stable_per_purpose_but_not_cross_linkable() -> None:
    now = datetime(2026, 8, 6, tzinfo=UTC)
    event = _event(occurred_at=now - timedelta(days=1))
    first = minimize_audit_event(event, secret=SECRET, now=now, retention_days=90)  # type: ignore[arg-type]
    second = minimize_audit_event(event, secret=SECRET, now=now, retention_days=90)  # type: ignore[arg-type]
...
def test_minimization_requires_strong_secret() -> None:
    ...
    with pytest.raises(ValueError, match="at least 32 bytes"):
        minimize_audit_event(event, secret=b"short", now=now, retention_days=90)  # type: ignore[arg-type]
