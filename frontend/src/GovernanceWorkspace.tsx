    ...
    <section className="workspace-foundation" aria-labelledby="workspace-title">
      ...
      {data && <>
        <div className="command-grid">
          <article className="surface command-panel"><header className="panel-heading"><div><p className="eyebrow">Frameworks</p><h2>Explicit coverage state</h2></div><span className="evidence-label">No inferred crosswalks</span></header><div className="integration-list">{data.frameworks.map((item) => <div className="integration-row" key={item.id}><span className={`integration-state state-${item.coverage === 'unmapped' ? 'disabled' : 'enabled'}`} aria-hidden="true" /><div><strong>{item.name}</strong><span>{item.coverage_label} · {item.kind}</span><small>{item.mapping_ids.length ? `${item.mapping_ids.join(', ')} · ` : ''}{item.note}</small></div></div>)}</div></article>
