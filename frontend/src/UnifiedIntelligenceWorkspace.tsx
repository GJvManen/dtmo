      ...
      <div className="intelligence-workspace-grid">
        ...
        <article className="surface intelligence-detail-panel">
          ...
          {detail && <div className="intelligence-detail">
            ...
            <section className="detail-section object-actions" aria-label="Object actions"><h4>Continue investigation</h4><div className="quick-grid"><a className="quick-action" href={`/workbench/analysis?item=${encodeURIComponent(detail.id)}`}><span aria-hidden="true">⌁</span><div><strong>Analyze &amp; enrich</strong><small>Open persisted IntelOwl/Cortex history for this canonical object.</small></div></a><a className="quick-action" href={`/workbench/sharing?item=${encodeURIComponent(detail.id)}`}><span aria-hidden="true">⇄</span><div><strong>Review &amp; share</strong><small>Open the governed review, approval and MISP export chain for this canonical object.</small></div></a></div><p className="boundary-copy">These pivots carry only the canonical object identifier. Analysis reloads server-authorized history and never executes an analyzer automatically; execution still requires <code>review:intelligence</code>. Sharing reloads server-authorized governance state and grants no review, share approval, export, publication or synchronization authority by navigation alone.</p></section>
